...
> PRD v1.3 WBS(Section 10) 기반 구현 이슈 분해.
...
- [x] ISSUE-030: list/board에서 open|done|drop status 필터 토큰 파싱 지원 _(track: product, P1, 1d)_ → PR #55
- [x] ISSUE-031: help 커맨드 및 빈 todo: 입력 시 상세 도움말 출력 _(track: product, P1, 0.5d)_ → PR #57
- [x] ISSUE-032: UX 명세에 맞는 응답 메시지 포맷 통일 _(track: product, P0, 1.5d)_ → PR #53
- [x] ISSUE-033: add 시 존재하지 않는 프로젝트 자동 생성 (shared) _(track: product, P1, 1d)_ → PR #59
- [x] ISSUE-034: move 커맨드에서 /s 없이 section 직접 지정 지원 _(track: product, P1, 0.5d)_ → PR #61
- [x] ISSUE-035: 에러 메시지 UX 명세 정합 _(track: product, P1, 1d)_ → PR #63
- [x] ISSUE-036: set-private 에러 메시지에 Slack 멘션 포맷 적용 _(track: product, P2, 0.5d)_ → PR #72
- [x] ISSUE-037: parser 단위 테스트 보강 -- 엣지 케이스 _(track: platform, P1, 1d)_ → PR #65
- [x] ISSUE-038: E2E 테스트 보강 -- scope 필터 및 다중 사용자 시나리오 _(track: platform, P1, 1d)_ → PR #67
...
- [x] ISSUE-041: bridge handler 버그 수정 (ctx.commandBody 중복, 에러 노출, senderId 폴백, 네트워크 에러) _(track: platform, P0, 0.5d)_ → PR #80
- [x] ISSUE-042: /todo project create 커맨드 구현 (shared/private 명시적 프로젝트 생성) _(track: product, P0, 1d)_ → PR #82
- [x] ISSUE-043: /todo project rename 커맨드 구현 (이름 변경 + 중복 블락) _(track: product, P0, 1d)_ → PR #84
...
### ISSUE-042: /todo project create 커맨드 구현 (shared/private 명시적 프로젝트 생성)
- Track: product
- PRD-Ref: PRD#5.11, PRD#3.1, PRD#9 (AC: project create 항목)
- Priority: P0
- Estimate: 1d
- Status: done
- Owner: claude
- Branch: `issue/ISSUE-042-project-create`
- GH-Issue: #81
- PR: #82

#### Goal
PRD 5.11에 따라 `/todo project create <name> [shared|private]` 커맨드를 구현한다. visibility 기본값은 shared이며, shared는 전역 유니크, private는 same owner 내 유니크 검증을 수행한다.

#### Scope
- In:
  - `src/openclaw_todo/cmd_project_create.py` -- 새 핸들러 생성
  - `src/openclaw_todo/dispatcher.py` -- `project create` 서브커맨드 라우팅 추가
  - `src/openclaw_todo/db.py` (필요 시) -- 프로젝트 생성 헬퍼 재사용 또는 추가
- Out:
  - 기존 `cmd_add.py`의 auto-create 로직 변경 없음
  - `cmd_project_set_shared.py` / `cmd_project_set_private.py`는 별도 유지

#### Acceptance Criteria (DoD)
- [ ] `/todo project create Shopping` 입력 시 shared 프로젝트 "Shopping" 생성 (owner_user_id=NULL)
- [ ] `/todo project create Personal private` 입력 시 private 프로젝트 "Personal" 생성 (owner_user_id=sender)
- [ ] 동일 이름의 shared 프로젝트가 이미 존재하면 에러: `Cannot create: shared project "Shopping" already exists.`
- [ ] sender가 이미 같은 이름의 private 프로젝트를 보유하면 에러: `Cannot create: you already have a private project named "Personal".`
- [ ] 성공 응답: `Created project "Shopping" (shared)` 또는 `Created project "Personal" (private, owner:<@U1>)`
- [ ] visibility 인자가 shared/private 이외의 값이면 적절한 에러 반환

#### Tests
- [ ] Unit: `tests/test_cmd_project_create.py` -- shared 생성 성공
- [ ] Unit: `tests/test_cmd_project_create.py` -- private 생성 성공
- [ ] Unit: `tests/test_cmd_project_create.py` -- shared 중복 에러
- [ ] Unit: `tests/test_cmd_project_create.py` -- private 같은 owner 중복 에러
- [ ] Unit: `tests/test_cmd_project_create.py` -- 잘못된 visibility 값 에러
- [ ] Integration: `tests/test_e2e.py` -- project create 전체 흐름
- Test Command: `uv run pytest tests/test_cmd_project_create.py tests/test_e2e.py -q`

#### Observability (Minimal)
- [ ] Logs: `event_logger`에 `project.create` 이벤트 기록 (name, visibility, owner)

#### Rollback
- 핸들러 파일 제거 + dispatcher 라우팅 원복으로 원상 복구

#### Dependencies / Blockers
- 없음 (독립 작업). ISSUE-033 (auto-create)과 로직 겹침 없도록 프로젝트 생성 헬퍼 재사용 권장.

---

### ISSUE-043: /todo project rename 커맨드 구현 (이름 변경 + 중복 블락)
- Track: product
- PRD-Ref: PRD#5.12, PRD#3.1, PRD#3.2, PRD#9 (AC: project rename 항목)
- Priority: P0
- Estimate: 1d
- Status: done
- Owner: claude
- Branch: `issue/ISSUE-043-project-rename`
- GH-Issue: #83
- PR: #84

#### Goal
PRD 5.12에 따라 `/todo project rename <old> <new>` 커맨드를 구현한다. 프로젝트 resolve는 옵션 A(sender private 우선 -> shared) 정책을 따르며, 새 이름이 기존 프로젝트와 중복되면 에러로 블락한다.

#### Scope
- In:
  - `src/openclaw_todo/cmd_project_rename.py` -- 새 핸들러 생성
  - `src/openclaw_todo/dispatcher.py` -- `project rename` 서브커맨드 라우팅 추가
  - `src/openclaw_todo/project_resolver.py` (필요 시) -- resolve 재사용
- Out:
  - 기존 project 커맨드(list/set-private/set-shared/create)는 변경 없음

#### Acceptance Criteria (DoD)
- [ ] `/todo project rename Old New` 입력 시 프로젝트 이름 "Old" -> "New" 변경
- [ ] 프로젝트 resolve 시 옵션 A 정책 적용 (sender private 우선 -> shared)
- [ ] private 프로젝트는 owner만 rename 가능, 비 owner 시도 시 권한 에러
- [ ] shared -> shared rename 시 새 이름이 기존 shared와 중복이면 에러: `Cannot rename: shared project "New" already exists.`
- [ ] private -> private rename 시 같은 owner 내 새 이름 중복이면 에러: `Cannot rename: you already have a private project named "New".`
- [ ] 존재하지 않는 프로젝트 rename 시 에러: `Project "Unknown" not found.`
- [ ] 성공 응답: `Renamed project "Old" -> "New" (shared)` 또는 `Renamed project "Old" -> "New" (private)`
- [ ] `projects.updated_at` 갱신 확인

#### Tests
- [ ] Unit: `tests/test_cmd_project_rename.py` -- shared 프로젝트 rename 성공
- [ ] Unit: `tests/test_cmd_project_rename.py` -- private 프로젝트 rename 성공
- [ ] Unit: `tests/test_cmd_project_rename.py` -- shared 새 이름 중복 에러
- [ ] Unit: `tests/test_cmd_project_rename.py` -- private 같은 owner 새 이름 중복 에러
- [ ] Unit: `tests/test_cmd_project_rename.py` -- 존재하지 않는 프로젝트 에러
- [ ] Unit: `tests/test_cmd_project_rename.py` -- 비 owner가 private 프로젝트 rename 시 권한 에러
- [ ] Integration: `tests/test_e2e.py` -- project rename 전체 흐름
- Test Command: `uv run pytest tests/test_cmd_project_rename.py tests/test_e2e.py -q`

#### Observability (Minimal)
- [ ] Logs: `event_logger`에 `project.rename` 이벤트 기록 (old_name, new_name, visibility)

#### Rollback
- 핸들러 파일 제거 + dispatcher 라우팅 원복으로 원상 복구

#### Dependencies / Blockers
- ISSUE-042 이후 권장 (project create 구현 시 공통 유틸리티 확립 후 rename에서 재사용)

---

### ISSUE-044: /todo project delete 커맨드 구현 (태스크 존재 시 삭제 차단)
- Track: product
...
#### Scope
- In:
  - `src/openclaw_todo/cmd_project_delete.py` -- 새 핸들러 생성
  - `src/openclaw_todo/dispatcher.py` -- `project delete` 서브커맨드 라우팅 추가 + help 텍스트 업데이트
- Out:
  - 기존 project 커맨드(list/set-private/set-shared/create/rename)는 변경 없음
  - CASCADE 삭제(태스크 함께 삭제)는 지원하지 않음

#### Acceptance Criteria (DoD)
- [ ] `/todo project delete Shopping` 입력 시 태스크가 없으면 프로젝트 삭제
- [ ] 프로젝트 내에 태스크가 1개 이상 존재하면 에러: `Cannot delete: project "Shopping" still has N task(s). Remove or move them first.`
- [ ] 프로젝트 resolve 시 옵션 A 정책 적용 (sender private 우선 -> shared)
- [ ] private 프로젝트는 owner만 삭제 가능, 비 owner 시도 시 "not found" 반환
- [ ] 존재하지 않는 프로젝트 삭제 시 에러: `Project "Unknown" not found.`
- [ ] 성공 응답: `Deleted project "Shopping" (shared)` 또는 `Deleted project "Personal" (private)`
- [ ] help 커맨드에 project delete 설명 포함
- [ ] Inbox 프로젝트 삭제 차단 (시스템 프로젝트)

#### Tests
- [ ] Unit: `tests/test_cmd_project_delete.py` -- 빈 shared 프로젝트 삭제 성공
- [ ] Unit: `tests/test_cmd_project_delete.py` -- 빈 private 프로젝트 삭제 성공
- [ ] Unit: `tests/test_cmd_project_delete.py` -- 태스크 존재 시 삭제 차단 에러
- [ ] Unit: `tests/test_cmd_project_delete.py` -- 존재하지 않는 프로젝트 에러
- [ ] Unit: `tests/test_cmd_project_delete.py` -- 비 owner가 private 프로젝트 삭제 시 "not found"
- [ ] Unit: `tests/test_cmd_project_delete.py` -- 이벤트 로깅 검증
- [ ] Unit: `tests/test_cmd_project_delete.py` -- Inbox 삭제 차단
- Test Command: `uv run pytest tests/test_cmd_project_delete.py -q`

#### Observability (Minimal)
- [ ] Logs: `event_logger`에 `project.delete` 이벤트 기록 (name, visibility)

#### Rollback
- 핸들러 파일 제거 + dispatcher 라우팅 원복으로 원상 복구

#### Dependencies / Blockers
- ISSUE-042 (project create) 완료 후 작업 권장
...
Phase 1 (핵심 기능 정합 -- P0/P1):
  ...
  #042 (project create) -- 독립
  #043 (project rename) -- #042 이후 권장
  #044 (project delete) -- #042 이후 권장
