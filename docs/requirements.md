...
| **커맨드 접두사** | `todo:` 단일 접두사 (슬래시 커맨드 `/todo` 미지원) |
...
| C6 | 커맨드 접두사는 `todo:` 단일. `/todo`는 인식하지 않는다. |
...
| AC-07.4 | `/todo`로 시작하는 메시지는 인식하지 않는다 (의도적 미지원). | 자동 테스트 |
...
| FR-01.2 | `/todo`로 시작하는 메시지는 인식하지 않아야 한다 (슬래시 커맨드 미지원). | `/todo add test` 입력 시 무시 확인 |
